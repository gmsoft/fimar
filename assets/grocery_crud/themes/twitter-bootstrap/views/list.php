	...
	<table class="table table-bordered tablesorter table-striped">
		<thead>
			<tr>
				...
				<th class="<?php echo (!$short) ? 'no-sorter' : 'hidden-phone sorting' ?>">
		...
		<tbody>
			<?php
                        /* para achicar las listas  */
                        foreach($list as $num_row => $row){ 
                        $short = false;
                        ?>
			<tr class="<?php echo ($num_row % 2 == 1) ? 'erow' : ''; ?>">
				<?php foreach($columns as $column){?>
					<td class="<?php echo (!$short) ? 'no-sorter' : 'hidden-phone sorting' ?> <?php if(isset($order_by[0]) &&  $column->field_name == $order_by[0]){?>sorted<?php }?>">
